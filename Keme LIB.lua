--[[
    Keme Lib - Updated for Modern Roblox
    
    This library has been updated to work with modern Roblox APIs and features.
    All deprecated methods have been replaced with their modern equivalents.
    
    Features:
    - Modern service usage (game:GetService() instead of game.ServiceName)
    - Updated PlayerList with PlayerAdded/PlayerRemoving events
    - Enhanced UI elements with TextBox and Label support
...
function Library:CreateWindow(title, color)
    ...
    function WinTypes:CreateTab(name, players)
        ...
        function TabTypes:CreateGroupbox(name, side)
            ...
            function GroupTypes:CreateTextBox(name, placeholder, callback)
                ...
                -- TextBox Instances
                local title_textbox = Instance.new("TextLabel")
                local textbox_frame = Instance.new("Frame")
                local UIGradient_textbox = Instance.new("UIGradient")
                local textbox = Instance.new("TextBox")

                -- TextBox Properties
                title_textbox.Name = "title"
                title_textbox.Parent = container_2
                title_textbox.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                title_textbox.BackgroundTransparency = 1.000
                title_textbox.Position = UDim2.new(0, 10, 0, 51)
                title_textbox.Size = UDim2.new(0, 234, 0, 20)
                title_textbox.ZIndex = 0
                title_textbox.Font = Enum.Font.SourceSans
                title_textbox.Text = name
                title_textbox.TextColor3 = Color3.fromRGB(255, 255, 255)
                title_textbox.TextSize = 15.000
                title_textbox.TextStrokeTransparency = 0.000
                title_textbox.TextXAlignment = Enum.TextXAlignment.Left
                title_textbox.TextYAlignment = Enum.TextYAlignment.Top

                textbox_frame.Name = "textbox_frame"
                textbox_frame.Parent = title_textbox
                textbox_frame.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
                textbox_frame.BorderColor3 = Color3.fromRGB(8, 8, 8)
                textbox_frame.Position = UDim2.new(0, 0, 0, 22)
                textbox_frame.Size = UDim2.new(0, 234, 0, 20)

                UIGradient_textbox.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 255, 255)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(167, 167, 167))}
                UIGradient_textbox.Rotation = 90
                UIGradient_textbox.Parent = textbox_frame
            ...
            function GroupTypes:CreateLabel(name, text)
                name = name or "New Label"
                text = text or "Label Text"

                -- Label Instances
                local label = Instance.new("TextLabel")
                local UIGradient_label = Instance.new("UIGradient")

                -- Label Properties
                label.Name = "label"
                label.Parent = container_2
                label.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
                label.BorderColor3 = Color3.fromRGB(8, 8, 8)
                label.Position = UDim2.new(0.0399999991, 0, 0.273542613, 0)
                label.Size = UDim2.new(0, 234, 0, 20)
                label.ZIndex = 0
                ...
                UIGradient_label.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 255, 255)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(167, 167, 167))}
                UIGradient_label.Rotation = 90
                UIGradient_label.Parent = label

                -- Label Types
                local LabelTypes = {}
                
                function LabelTypes:SetText(newText)
        ...
        function TabTypes:CreatePlayerList(showbuttons, events)
            ...
            local function CreatePlayerCard(Name, UserId, MemberShipType, TeamColor)
                ...
                local Pfp = Players:GetUserThumbnailAsync(UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
            ...
            Players.PlayerRemoving:Connect(function(Plr)
                if insertedplayers[Plr.UserId] then
    ...
    function WinTypes:SetTheme(newTheme)
        -- Theme support for modern UI
        if newTheme == "Dark" then
            color = Color3.fromRGB(19, 119, 255)
        elseif newTheme == "Light" then
            color = Color3.fromRGB(100, 149, 237)
        elseif newTheme == "Neon" then
            color = Color3.fromRGB(255, 0, 255)
        end
    end

    function WinTypes:AddNotification(message, duration)
        duration = duration or 3
        -- Modern notification system
        local notification = Instance.new("Frame")
        notification.Name = "Notification"
        notification.Parent = BracketV2
        ...
        local tween = TweenService:Create(notification, tweenInfo, {Position = UDim2.new(1, -300, 0, 20)})
        tween:Play()
...
function Library:CreateNotification(message, duration)
    duration = duration or 3
    local notification = Instance.new("Frame")
    notification.Name = "Notification"
...
function Library:CreateModernButton(text, callback)
    local button = Instance.new("TextButton")
