...
local library = {
    ...
    opening = false;
    hasInit = false;
    cheatname = startupArgs.cheatname or 'Keme Lib';
...
function library:init()
    ...
    -- Add method to create custom sections
    function self:CreateCustomSection(tab, name, order)
        return tab:AddSection(name, order or 1)
    end

    -- Add method to create custom tabs
    function self:CreateCustomTab(window, name, order)
        return window:AddTab(name, order or 1)
    end

    -- Add method to create custom watermark
    function self:CreateCustomWatermark(text, position, enabled)
        if self.watermark then
            self.watermark:SetText(text or "Keme Lib")
            if position then
                self.watermark.lock = position
            end
            if enabled ~= nil then
                self.flags.watermark_enabled = enabled
            end
        end
    end

    -- Add method to create custom themes
    function self:CreateCustomTheme(name, themeData)
        if typeof(themeData) == 'table' then
            table.insert(self.themes, {
                name = name,
                theme = themeData
            })
            return true
    ...
    function self.NewWindow(data)
        ...
        do
            ...
            -- Add method to get current color as hex
            function window.colorpicker:GetHexColor()
                local r, g, b = math.floor(self.color.R * 255), math.floor(self.color.G * 255), math.floor(self.color.B * 255)
                return string.format("#%02X%02X%02X", r, g, b)
            end

            -- Add method to get current color as RGB values
            function window.colorpicker:GetRGBColor()
                local r, g, b = math.floor(self.color.R * 255), math.floor(self.color.G * 255), math.floor(self.color.B * 255)
                return r, g, b
            end

            -- Add method to set color from hex
            function window.colorpicker:SetHexColor(hex)
                if typeof(hex) == 'string' and hex:match("^#[0-9A-Fa-f]{6}$") then
                    local r = tonumber(hex:sub(2,3), 16) / 255
                    local g = tonumber(hex:sub(4,5), 16) / 255
                    local b = tonumber(hex:sub(6,7), 16) / 255
                    self:Visualize(Color3.new(r, g, b), self.trans)
                end
            end

            -- Add method to set color from RGB values
            function window.colorpicker:SetRGBColor(r, g, b)
                if typeof(r) == 'number' and typeof(g) == 'number' and typeof(b) == 'number' then
                    ...
                    self:Visualize(Color3.new(r/255, g/255, b/255), self.trans)
                end
            end
    ...
    do
        self.watermark = {
            ...
            position = newUDim2(0,0,0,0);
            refreshrate = 25;
            customText = "Keme Lib | v1.0 | " .. localplayer.Name;
        }
        ...
        function self.watermark:Update()
            ...
            if library.flags.watermark_enabled then
                ...
                local displayText = self.customText or table.concat(text,' | ')
                self.objects.text.Text = displayText
                
                -- Calculate size with padding
                local textSize = self.objects.text.TextBounds.X
                local padding = 10
                self.objects.background.Size = newUDim2(0, textSize + padding, 0, 17)

                local size = self.objects.background.Object.Size;
                local screensize = workspace.CurrentCamera.ViewportSize;

                -- Improved positioning with better calculations
                self.position = (
        ...
        -- Add method to set custom watermark text
        function self.watermark:SetText(text)
            self.customText = text
        end

        -- Add method to toggle watermark elements
        function self.watermark:ToggleElement(index, enabled)
            if self.text[index] then
                self.text[index][2] = enabled
            end
        end
...
function library:CreateSettingsTab(menu)
    ...
    local basicSection = settingsTab:AddSection('Basic Settings', 1);
    
    basicSection:AddBind({text = 'Open / Close', flag = 'togglebind', nomouse = true, noindicator = true, bind = Enum.KeyCode.RightShift, callback = function()
        library:SetOpen(not library.open)
    end});

    basicSection:AddToggle({text = 'Disable Movement If Open', flag = 'disablemenumovement', callback = function(bool)
    ...
    basicSection:AddButton({text = 'Rejoin Server', confirm = true, callback = function()
        game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId);
    end})

    basicSection:AddButton({text = 'Copy Join Script', callback = function()
        setclipboard(([[game:GetService("TeleportService"):TeleportToPlaceInstance(%s, "%s")]]):format(game.PlaceId, game.JobId))
    end})

    basicSection:AddButton({text = 'Unload', confirm = true, callback = function()
        library:Unload();
    end})
    ...
    local uiSection = settingsTab:AddSection('UI Customization', 3);
    
    -- Keybind Indicator Settings
    uiSection:AddSeparator({text = 'Keybind Indicator'});
    uiSection:AddToggle({text = 'Keybind Indicator', flag = 'keybind_indicator', callback = function(bool)
        library.keyIndicator:SetEnabled(bool);
    end})
    uiSection:AddSlider({text = 'Position X', flag = 'keybind_indicator_x', min = 0, max = 100, increment = .1, value = .5, callback = function()
    ...
    uiSection:AddSlider({text = 'Position Y', flag = 'keybind_indicator_y', min = 0, max = 100, increment = .1, value = 35, callback = function()
        library.keyIndicator:SetPosition(newUDim2(library.flags.keybind_indicator_x / 100, 0, library.flags.keybind_indicator_y / 100, 0));    
    end});

    -- Watermark Settings
    uiSection:AddSeparator({text = 'Watermark'})
    uiSection:AddToggle({text = 'Enabled', flag = 'watermark_enabled'});
    uiSection:AddList({text = 'Position', flag = 'watermark_pos', selected = 'Custom', values = {'Top', 'Top Left', 'Top Right', 'Bottom Left', 'Bottom Right', 'Custom'}, callback = function(val)
        library.watermark.lock = val;
    end})
    uiSection:AddSlider({text = 'Custom X', flag = 'watermark_x', suffix = '%', value = 6.1, min = 0, max = 100, increment = .1});
    uiSection:AddSlider({text = 'Custom Y', flag = 'watermark_y', suffix = '%', value = 1.2, min = 0, max = 100, increment = .1});
    ...
    local themeSection = settingsTab:AddSection('Theme Settings', 4);
